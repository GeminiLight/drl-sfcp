...
from .utils import path_to_edges
...
class Attribute(object):
    ...
    def __init__(self, name, owner, type, *args, **kwargs):
        ...
        if self.generative:
            ...
            assert self.distribution in ['normal', 'uniform', 'customized']
    ...
    def _generate_data_with_dist(self, network):
        ...
        if self.distribution == 'normal' and self.dtype == 'float':
            attribute_data = np.random.normal(self.loc, self.scale, size).tolist()
        elif self.distribution == 'uniform' and self.dtype == 'int':
            attribute_data = np.random.randint(self.low, self.high, size).tolist()
        elif self.distribution == 'uniform' and self.dtype == 'float':
            attribute_data = np.random.uniform(self.low, self.high, size).tolist()
        else:
...
class ExtremaMethod:
    ...
    def check(self, vn, pn, vid, pid, method='ge'):
        return True
...
class EdgeResourceAttribute(Attribute, EdgeMethod, ResourceMethod):
    ...
    def check(self, vn, pn, vl_id, pl_id, method='ge'):
        v, p = vn.edges[vl_id], pn.edges[pl_id]
        return super()._check_one_element(v, p, method)
